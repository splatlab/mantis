...
MSTMerger::MSTMerger(std::string prefixIn, spdlog::logger *loggerIn, uint32_t numThreads,
    ...
    std::string colorFile = prefix + "newID2oldIDs";
    std::ifstream cp(colorFile);
    ...
    uint64_t c1len{static_cast<uint64_t >(ceil(log2(ccCnt[0])))}, c2len{static_cast<uint64_t >(ceil(log2(ccCnt[1])))};
...
//        cp.read(reinterpret_cast<char *>(&c2), sizeof(uint64_t));
    ...
    cp.close();
    std::cerr << "\r";
    logger->info("Read {} color pairs from file", colorCntr);
...
bool MSTMerger::buildEdgeSets() {
    ...
    int64_t globalCntr{0};
    pc_t ptCntr_;
    pc_init(&ptCntr_, &globalCntr, nThreads, 1000000);
...
//        logger->info("Iterating over cqf & building edgeSet ...");
        ...
        for (uint32_t i = 0; i < nThreads; ++i) {
            threads.emplace_back(std::thread(&MSTMerger::buildPairedColorIdEdgesInParallel, this, i,
                                             ...
                                             std::ref(cnts[i]),
                                             std::ref(maxId), std::ref(numOfKmers), (void *)(&ptCntr_),
                                             std::ref(popularEdges)));
        }
        for (auto &t : threads) { t.join(); }
        pc_sync(&ptCntr_);
        if (numOfKmers != globalCntr) {
            std::cerr << "The mutex is wrong:: Kmer count is {}, while it should be {}\n";
        }
...
void MSTMerger::buildPairedColorIdEdgesInParallel(uint32_t threadId,
                                            ...
                                            uint64_t &curFileIdx,
                                            std::uint64_t& cnt,
                                            uint64_t &maxId, uint64_t &numOfKmers, void *pt_cntr,
                                            spp::sparse_hash_map<std::pair<uint64_t , uint64_t >, uint64_t, Custom_Pair_Hasher > &popularEdges) {
...
//        uint64_t curEqId = keyObject.count - 1;
    ...
    appendStore();
    pc_t *pc_counter = (pc_t*)pt_cntr;
    pc_add(pc_counter, kmerCntr);
    counterMutex.lock();
...
bool MSTMerger::calculateMSTBasedWeights() {
    ...
    for (uint64_t mstIdx = 0; mstIdx < 2; ++mstIdx) {
        ...
        if (ccCnt[mstIdx]*ceil(log2(toBeMergedNumOfSamples[mstIdx]+1)) < 64*2*nThreads) {
            ...
            for (uint64_t partition = 0; partition < 2; ++partition) {
                std::vector<std::thread> threads;
                for (uint64_t t = 0; t < nThreads; ++t) {
        ...
        logger->info("MST {}: Done counting set-bits for {} cc nodes.", mstIdx, ccSetBitCnts[mstIdx].size());
    }
    ...
    auto store_edge_weight = [&](auto pair, auto w) {
        if (w <= maxWeightInFile) {
            ...
            if (bufferCurrIndices[w] == bufferEndIndices[w]) {
                uint64_t startIdx = w == 0?0:bufferEndIndices[w-1];
        ...
        if (outputMstEdges.size() > maxAllowedCnt or minheap.empty()) {
            ...
            for (uint64_t mstIdx = 0; mstIdx < 2; mstIdx++) {
               ...
               /* logger->info("Total memory consumed by all the MPH tables = {} MB.",
                ...
                std::vector<std::thread> threads;
                for (uint64_t threadId = 0; threadId < nThreads; ++threadId) {
...
//    usleep(10000000);
    ...
    for (uint64_t i = 0; i < colorPairs[0].size(); i++) {
        ...
        if (w <= maxWeightInFile) {
            ...
            uint64_t &idx = ccBitsBucketCnt[w-1];
            idx--;
            ccBits[idx] = i;
            memoryDummies++;
        }
    }
    logger->info("Done calculating weight of {} dummy edges. {} with weight <= {} and {} with weight greater.", colorPairs[0].size(), fileDummies, maxWeightInFile, memoryDummies);
    ...
    for (uint64_t w = 0; w < outWeightFile.size(); w++) {
        uint64_t startIdx = w == 0?0:bufferEndIndices[w-1];
...
void MSTMerger::kruskalMSF(AdjList * adjListPtr) {
    ...
    uint64_t bucketsInFile = std::min(maxWeightInFile, bucketCnt);
    for (uint64_t bucketCntr = 0; bucketCntr < bucketsInFile; bucketCntr++) {
        w = bucketCntr + 1;
    ...
    std::cerr << "\n";
    logger->info("Done with edges with weight <= {} => {} edges processed and {} were selected with total weight of {}", maxWeightInFile, edgeCntr, selectedEdgeCntr, mstTotalWeight);
    for (uint64_t bucketCntr = bucketsInFile; bucketCntr < bucketCnt; bucketCntr++) {
    ...
    if (selectedEdgeCntr != num_colorClasses - 1) {
        logger->error("Kruskal is unable to output one tree. The color graph is not ONE connected component.");
        std::exit(3);
    }
...
*/
        ...
        if (deltas.size() >= deltasKeptInMem) {
            ...
            for (auto &v : deltas) {
                ...
                for (uint64_t cntr = 0; cntr < v.deltaVals.size(); cntr++) {
