...
class ColoredDbg {
	public:
		...
		// Methods for memory efficient (mostly disk-based) query.
		...
		void get_query_color_id_distribution(std::vector<uint64_t> &kmers, uint64_t startIdx, uint64_t endIdx,
												std::unordered_map<uint64_t, uint64_t> &freq);

		// Get the sample distribution from the color-id distribution at
		// colorIdDist[startIdx: endIdx).
		void get_query_sample_distribution(std::vector<std::pair<uint64_t, uint64_t>> &colorIdDist,
											uint64_t startIdx, uint64_t endIdx,
...
ColoredDbg<qf_obj, key_obj> ::
	...
	start_time_(std::time(nullptr))
	{
		...
		std::map<uint32_t, std::string> sortedFiles;
		for (std::string file : colorClassFiles)
		{
			uint32_t fileID = std::stoi(first_part(last_part(file, '/'), '_'));
...
template<typename qf_obj, typename key_obj>
std::vector<uint64_t> ColoredDbg<qf_obj, key_obj>::
	find_samples(std::unordered_set<uint64_t> &kmerSet, uint32_t threadCount)
{
	// Sort the query k-mer hashes; distribute the sorted kmer-list to
	// 'threadCount' number of threads; and then each thread reads in the
	// color-ids corresponding to the k-mers assigned to them.

	std::vector<uint64_t> kmers(kmerSet.begin(), kmerSet.end());
	sort(kmers.begin(), kmers.end());

	std::vector<std::thread> T;
	std::vector<std::unordered_map<uint64_t, uint64_t>> colorIdFreq;
	std::vector<std::vector<uint64_t>> sampleFreq;

	T.reserve(threadCount);

	colorIdFreq.resize(threadCount);
	uint64_t perThreadTask = kmers.size() / threadCount;

	for(uint32_t t = 0; t < threadCount; ++t)
		T.emplace_back(&ColoredDbg<qf_obj, key_obj>::get_query_color_id_distribution, this,
						std::ref(kmers), t * perThreadTask,
						t < threadCount - 1 ? (t + 1) * perThreadTask : kmers.size(),
						std::ref(colorIdFreq[t]));


	// Aggregate the color-id distributions generated by the threads.

	std::unordered_map<uint64_t, uint64_t> aggregateColorIdDist;
	for(uint32_t t = 0; t < threadCount; ++t)
	{
		T[t].join();
		for(auto p = colorIdFreq[t].begin(); p != colorIdFreq[t].end(); ++p)
			aggregateColorIdDist[p -> first] += p -> second;
	...
	// Sort the collected color-ids; distribute the sorted color-id list to
	// 'threadCount' number of threads; and then each thread reads in the
	// color-classes corresponding to the color-ids assigned to them; and
	// accumulates sample distribution from the color-classes.


	std::vector<std::pair<uint64_t, uint64_t>> colorIdDist(aggregateColorIdDist.begin(), aggregateColorIdDist.end());
	std::sort(colorIdDist.begin(), colorIdDist.end());

	sampleFreq.resize(threadCount);
	perThreadTask = colorIdDist.size() / threadCount;

	for(uint32_t t = 0; t < threadCount; ++t)
		T.emplace_back(&ColoredDbg<qf_obj, key_obj>::get_query_sample_distribution, this, std::ref(colorIdDist),
						t * perThreadTask, t < threadCount - 1? (t + 1) * perThreadTask : colorIdDist.size(),
						std::ref(sampleFreq[t]));

	
	// Aggregate the sample distribution generated by the threads.

	std::vector<uint64_t> aggregateSampleDist(num_samples, 0);
	for(uint32_t t = 0; t < threadCount; ++t)
	{
		T[t].join();
		for(uint64_t sampleNum = 0; sampleNum < num_samples; ++sampleNum)
			aggregateSampleDist[sampleNum] += sampleFreq[t][sampleNum];

		sampleFreq[t].clear();
	}

	return aggregateSampleDist;
}
...
template<typename qf_obj, typename key_obj>
void ColoredDbg<qf_obj, key_obj>::
	get_query_color_id_distribution(std::vector<uint64_t> &kmers, uint64_t startIdx, uint64_t endIdx,
										std::unordered_map<uint64_t, uint64_t> &freq)
{
	for(uint64_t i = startIdx; i < endIdx; ++i)
	{
		key_obj key(kmers[i], 0, 0);
...
template<typename qf_obj, typename key_obj>
void ColoredDbg<qf_obj, key_obj>::
	get_query_sample_distribution(std::vector<std::pair<uint64_t, uint64_t>> &colorIdDist, uint64_t startIdx,
									uint64_t endIdx, std::vector<uint64_t> &sampleCount)
{
	BitVectorRRR bitVec;
	uint64_t currBucket = std::numeric_limits<uint64_t>::max();
	sampleCount.resize(num_samples, 0);

	for(uint64_t i = startIdx; i < endIdx; ++i)
	{
		uint64_t colorId = colorIdDist[i].first, count = colorIdDist[i].second;
	...
	move_index(std::string source, std::string destination, spdlog::logger *console)
{
	...
	for(uint32_t i = 0; i < colorClassFiles.size(); ++i)
