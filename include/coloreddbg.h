...
//        dbgs.resize(other.dbgs.size());
    ...
    const CQF<key_obj> *get_cqf() const { return curDbg.get(); }//&dbg; }

    const CQF<key_obj> *get_current_cqf() const { return curDbg.get(); }
    ...
    uint32_t seed() const { return curDbg.seed(); }

    uint64_t range() const { return curDbg.range(); }
    ...
    void setUpperboundPerBlock(uint64_t keybits, double blockSizeInGig=2) {
        ...
        // 2^(keybits-r) = (std::ceil(colorBits/r)+2) * cqfSlotCnt
        if (keybits > 64) {
            std::cerr << "ERROR! keybits cannot go over 64 bits.\n";
            std::exit(3);
        }
    ...
    uint64_t getColorIdSlotCnt(uint64_t colorId) {
        return static_cast<uint64_t >(ceil(ceil(log2(colorId))/remainder));
    }
...
private:
    ...
    uint64_t get_next_available_id();
...
ColoredDbg(ColoredDbg<qf_obj, key_obj> &cdbg1, ColoredDbg<qf_obj, key_obj> &cdbg2, std::string &prefixIn,
           int flag):
        bv_buffer(),
        prefix(prefixIn),
...
template<class qf_obj, class key_obj>
ColoredDbg<qf_obj, key_obj>::ColoredDbg(uint64_t nqf, uint64_t keybits) :
...
//        std::cerr << first_second_minimizers.first << " ";
        ...
        // Progress tracker
        if (counter % approximateKmerCnt == 0) {
