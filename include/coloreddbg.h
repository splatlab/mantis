...
class ColoredDbg {
    ...
    default_cdbg_bv_map_t&
		...
		uint64_t color_class_id(uint64_t kmer) const
		{
		...
		class color_class {
		public:
			color_class(const ColoredDbg &_cdbg, uint64_t _ccid)
				: num_samples(_cdbg.get_num_samples()),
				eqcbv_offset(((_ccid - 1) % mantis::NUM_BV_BUFFER) * num_samples),
				eqcbv(_cdbg.eqclasses[(_ccid - 1) / mantis::NUM_BV_BUFFER])
					{}
			
			// Return the 64-bits at bit-offset offset.  Correctly handles the end of the color class.
			uint64_t get_uint64(uint64_t offset) const {
				uint64_t len = std::min((uint64_t)64, num_samples - offset);
				return eqcbv.get_int(eqcbv_offset + offset, len);
			}

			// Return the indexth bit of this color class vector
			bool operator[](uint64_t index) const {
				uint64_t wrd = get_uint64(index / 64);
				return (wrd >> (index % 64)) & 0x01;
			}
			
			// Returns the length of a color class vector.
			uint64_t size() const {
				return num_samples;
			}
		...
		private:
			...
			uint64_t eqcbv_offset;
			const BitVectorRRR & eqcbv;
		};
		
		const color_class get_color_class(uint64_t ccID) const {
			return color_class(*this, ccID);
		}
